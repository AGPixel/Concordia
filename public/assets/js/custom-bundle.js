...
(function ($) {
    ...
    function hoverImage($el = $(document)) {
        const { motionHoverEffect } = dsnGrid;
    ...
    function preloader() {
        ...
        window.addEventListener('DOMContentLoaded', function () {
            ...
            tl.to(present, 3, {
                ...
                .to(preloader.find('> *:not(.bg-load)'), { autoAlpha: 0 })
                ...
                .to(preloader.find('.bg-load'), {
                    ...
                    attr: { d: dsnGrid.getData(preloader.find('.bg-load .separator__path').get(0), 'to') },
                    ease: "Power4.easeInOut",
                    duration: 1.5
                }, '-=1.5')
                .fromTo("#main_root", 1, { y: 400 }, { y: 0, clearProps: true, ease: Expo.easeInOut }, "-=1.2")
                .call(function () {
                    preloader.remove();
    ...
    function menuInit() {
        ...
        const TransEnd = () => reserved.getValue() && menu.querySelector('.primary-nav')?.classList.add('open');
        ...
        const toggleClick = function () {
            ...
            if (!reserved.getValue()) {
                dsnGrid.svgAnimate.up(targets.svg, TransEnd)
                    .to("#dsn-scrollbar", { y: -200, duration: 1, ease: 'power4.in', }, "top")
                    .set(targets.backgroundMain, { autoAlpha: 1 }, "center");
                onCompleteAnimate(this);
            } else
                dsnGrid.svgAnimate.down(targets.svg, () => onCompleteAnimate(this))
                    .to("#dsn-scrollbar", { y: 0, clearProps: "y", duration: 1, ease: 'power4', }, "-=1")
                    .set(targets.backgroundMain, { autoAlpha: 0 }, "center");
        ...
        new Promise((resolve) => setTimeout(() => resolve(), 300))
            .then(() => dsnGrid.spltting.Char(targets.toggle?.querySelector('.text-menu')))
            .then(() => dsnGrid.spltting.Char(targets.toggle?.querySelector('.text-open')))
            .then(() => dsnGrid.spltting.Char(targets.toggle?.querySelector('.text-close')))
            .then(() => {
                ...
                gsap.to(menu, { yPercent: 0, autoAlpha: 0, delay: 1, clearProps: true });
            });
        ...
        targets.toggle?.addEventListener('click', toggleClick);
        targets.subMenu.on('click', handleClickSubMenu);
        targets.back.on('click', handleClickBackMenu);

        dsnGrid.killAjax(function () {
            targets.toggle?.removeEventListener('click', toggleClick);
    ...
    async function Isotope($el = $(document)) {
        ...
        await $el.find(".dsn-isotope:not(.use-filter)").each(function () {
            if ($(this).parent('.root-posts').length)
                return;
                ...
                if (dsnData.length - 1 === $index && $type === "src")
                    $args.onComplete = () => {
                        $('.swiper-container.swiper-initialized').each(function () {
                            ...
                            if (!swiper?.passedParams?.loop) return;
                            swiper.loopDestroy();
                            swiper.loopCreate();
                            swiper.update();
                            swiper.updateSlides();
    ...
    function effectScroller() {
        ...
        return {
             ...
             */
            isScroller: function () {
                return dsnParam?.scrollbar?.smooth;
            },


            start: function () {
                $body.removeClass(locked_scroll);
                dsnGrid.scrollTop(0, { duration: 0.01 });
            ...
            locked: function () {
                ...
                this.isScroller() && lenisScroll?.destroy();
            },
    ...
    function projectSlider() {
        return {
            ...
            swiper: function ($id, $obj) {
                ...
                const renderBullet = swiperPaginate.hasClass('dsn-swiper-img') ?
                    function (index, className) {
                        return '<span class="' + className + '">' + this.slides[index].querySelector('img')?.outerHTML + '</span>'
                    } : null;
    ...
    const activeNav = function ($index) {
        ...
        prev.get($index)?.classList.add('dsn-active');
        next.get($index)?.classList.add('dsn-active');
    }
    ...
    function sliderPortfolio($el = $(document)) {
        ...
        const initSlider = async function () {
            ...
            await this.querySelectorAll('.content-slider .slide-item').forEach(($item, $index) => {
                ...
                const pushItem = `<div class="dsn-slide d-flex align-items-center justify-content-center">${img.prop('outerHTML')}<h6 class="sm-title-block">${slideContent.querySelector('.title-lg')?.innerText}</h6></div>`;
        ...
        function progressBar(swiper, newNum) {
            gsap.to(this.querySelector(".progress-nav .progress-w-after"), 1, { width: ((Number.parseInt(newNum) + 1) / swiper.slides.length) * 100 + "%" });
        }
        ...
        function slideChange(swiper) {
                ...
                animate = {
                    show: {
                        autoAlpha: 1, y: 0, stagger: {
                            amount: 0.3,
                            from: "start"
                        }, ease: "back.out(4)", rotation: 0, scale: 1, yoyo: true
                    },
                    hide: {
                        autoAlpha: 0,
                        y: !$isRight ? "25%" : "-25%",
            ...
            if (num)
                num.innerHTML = current > 9 ? current : '0' + current;
        ...
        const NavSwiper = function (container, swiper) {
            ...
            const navSlider = new Swiper(navContainer, {
                speed: swiper.passedParams?.speed || 1000,
                loop: swiper.passedParams?.loop || false,
                touchRatio: 0.2,
        ...
        $el.find(".main-slider:not(.dsn-swiper-initialized)").each(function () {
            ...
            initSlider.bind(this)()
                ...
                .then(function (swiper) {
                    ...
                    if (this.classList.contains("hide-bg"))
                        gsap.to(this.querySelector(".bg-container"), { opacity: 1, delay: 2 });
                    dsnGrid.killAjax(function () {
    ...
    function mouseCirMove($off) {
        ...
        const defaultEl = $element.css(['opacity', 'width', 'height', 'borderColor', 'background']),
            { stop, run } = { stop: () => $body.addClass(mouseStop), run: () => $body.removeClass(mouseStop) };
        ...
        dsnGrid.mouseHover(".c-hidden , .social-side a , .next-arrow , .prev-arrow , .dsn-btn.vid", {
            enter() {
                ...
                gsap.to($element, 0.5, { width, height, opacity: 0, x, y, xPercent: 0, yPercent: 0 });
                gsap.to(inner, 0.1, { opacity: 0 });
            },
            leave() {
                run();
                gsap.to($element, 0.5, { ...defaultEl, xPercent: -50, yPercent: -50 });
                gsap.to(inner, 0.1, { opacity: 1 });
            }
        });
